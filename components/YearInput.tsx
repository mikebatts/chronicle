...
export default function YearInput({ onSubmit, disabled, previousGuesses, answerYear, guessYears }: YearInputProps) {
  ...
  return (
    <div className="w-full">
      ...
      {previousGuesses.length > 0 && (
        <div className="flex flex-col items-center gap-2 mb-4">
          {previousGuesses.map((row, rowIdx) => (
            <div key={rowIdx} className="flex gap-2 w-full max-w-[340px] rounded-lg p-1">
              {row.map((df, colIdx) => (
                <div
                  key={colIdx}
      ...
      {/* Directional hint below the grid */}
      {(() => {
        const latestGuess = guessYears[guessYears.length - 1];
        const hasWrongGuess = guessYears.length > 0 && latestGuess !== answerYear;
        if (!hasWrongGuess) return null;
        const direction = latestGuess < answerYear ? "↑ higher" : "↓ lower";
        return (
          <div className="text-xs text-gray-500 text-center mb-3 italic">
            {direction}
          </div>
        );
      })()}
